...
COLOR_MAP = {
    "🍼 Formula (mL)": "#0284c7", 
    "🤱 Breast Milk (mL)": "#fb7185", 
    "💧 Wet Diaper (Cnt)": "#3b82f6",
    "🚽 Poop (Cnt)": "#d97706", 
    "🧴 Pumping (mL)": "#8b5cf6", 
...
with tab1:
    ...
    if not today_24h_df.empty:
        ...
        for _, row in norm_today_df.iterrows():
            ...
            event_name = str(row['Event Type'])
            if pd.notna(val) and "(Cnt)" not in event_name:
                val_str = f"{int(val)}" if float(val).is_integer() else f"{val:.1f}"
                timeline_annotations.append(dict(
                    x=row['DateTime'],
                    y=row['Short_Event'],
                    text=f"<b>{val_str}</b>",
                    showarrow=False,
        ...
        fig_today_timeline.update_layout(
            ...
            yaxis=dict(title=dict(text=""), showgrid=True, gridcolor="#f1f5f9", tickfont=dict(size=10.5), automargin=True)
        )
...
with tab2:
    ...
    if not milk_df.empty:
        ...
        fig_milk.add_trace(go.Scatter(
            ...
            y=total_per_x['Trend'], 
            mode='lines', 
            line=dict(color='#334155', width=2, shape='spline'), 
        ...
        fig_milk.add_trace(go.Bar(
            ...
            textposition="outside", textfont=dict(size=9.5, color='#334155'),
                    ...
                    if current_max_time and live_max_time and live_max_time > current_max_time:
                        ...
                        if not new_rows_df.empty:
                            ...
                            for _, r in new_rows_df.iterrows():
                                ...
                                notes_val = str(r['Notes / Details (Optional)']) if ('Notes / Details (Optional)' in r and pd.notna(r['Notes / Details (Optional)'])) else ""
                                new_data.append([now_timestamp, dt_str, str(r['Event Type']), r['Value (Optional)'] if pd.notna(r['Value (Optional)']) else "", notes_val, now_timestamp])
